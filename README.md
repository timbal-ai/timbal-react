...
- `TimbalV2Button` — removed for good in 3.0.0. Use the standard `Button` export from this package; it covers the same variants and renders the pill chrome via `shape="pill"`. (Several `TIMBAL_V2_*` **surface** token records — modal/card/switch/avatar chrome — remain public exports for catalog/list surfaces; only the button component is gone.)
